...
class VictoriaMetricsConnector(MetricsConnector):
    ...
    async def query_range(
        ...
        try:
            ...
            raise InvalidQuery(f"VictoriaMetrics query failed [{e.response.status_code}]: {e.response.text}") from e
        except httpx.TimeoutException as e:
            raise QueryTimeout("VictoriaMetrics query timed out") from e
        except httpx.RequestError as e:
            raise DataSourceUnavailable(f"Cannot reach VictoriaMetrics at {url}") from e